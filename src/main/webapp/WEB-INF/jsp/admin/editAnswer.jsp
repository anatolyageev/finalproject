...
<div class="container">
    ...
    <form action="controller" method="post" name="answer">
        ...
        <c:forEach items="${answerListEn}" var="answer" varStatus="loop">
            <div class="form-row">
                ...
                <div class="form-group col-md-6 mb-2">
                    ...
                    <textarea class="form-control" rows="2" id="newQuestionRu" placeholder="Enter text for new question"
                              name="${answer.id}_ru" required>${answersRu[answer.id]}</textarea>
                    <div class="valid-feedback">Valid.</div>
                    <div class="invalid-feedback">Please fill out this field.</div>
                </div>
                <div class="form-group col-md-6 mb-4">
                    <input class="form-check-input" type="checkbox" name="isCorrect"
                           value="${answer.id}" id="is-correct-answer"
                    <c:if test="${answer.correctAnswer}">
                           checked
                    </c:if>>
                    <label class="form-check-label" for="is-correct-answer"><fmt:message
                            key="answer.answer_correct_check_box"/></label>
                </div>
            </div>
        </c:forEach>
